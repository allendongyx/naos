...
const Contribute = ({ onRefreshData }: Props) => {
  ...
  const linkUrl = useMemo(() => {
    if (language.startsWith('zh_'))
      return 'https://docs.naos.one/zh-hans/guides/tools#ru-he-chuang-jian-zi-ding-yi-gong-ju'
    return 'https://docs.naos.one/guides/tools#how-to-create-custom-tools'
  }, [language])
  ...
  return (
    <>
      {isCurrentWorkspaceManager && (
        <div className='flex flex-col col-span-1 bg-primary-500 border-[0.5px] border-divider-subtle rounded-xl transition-all duration-200 ease-in-out cursor-pointer hover:bg-primary-600 hover:shadow-lg'>
          <div className='group grow rounded-t-xl' onClick={() => setIsShowEditCustomCollectionModal(true)}>
            <div className='shrink-0 flex items-center p-4 pb-3'>
              ...
              <div className='ml-3 text-sm font-semibold leading-5 text-white'>{t('tools.createCustomTool')}</div>
            </div>
          </div>
